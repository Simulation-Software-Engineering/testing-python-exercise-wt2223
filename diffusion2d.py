...
class SolveDiffusion2D:
    ...
    def initialize_domain(self, w=10., h=10., dx=0.1, dy=0.1):
        assert type(w) == float, "parameter w must be of type float"
        assert type(h) == float, "parameter h must be of type float"
        assert type(dx) == float, "parameter dx must be of type float"
        assert type(dy) == float, "parameter dy must be of type float"
        self.w = w
        self.h = h
        self.dx = dx
        self.dy = dy
    ...
    def initialize_physical_parameters(self, d=4., T_cold=300.0, T_hot=700.0):
        assert type(d) == float, "parameter d must be of type float"
        assert type(T_cold) == float, "parameter T_cold must be of type float"
        assert type(T_hot) == float, "parameter T_hot must be of type float"
        self.D = d
        self.T_cold = T_cold
        self.T_hot = T_hot
