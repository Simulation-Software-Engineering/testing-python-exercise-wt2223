...
class SolveDiffusion2D:
    ...
    def initialize_domain(self, w=10., h=10., dx=0.1, dy=0.1):
        assert type(w) == float, "w must be a float"
        assert type(h) == float, "h must be a float"
        assert type(dx) == float, "dx must be a float"
        assert type(dy) == float, "dy must be a float"
    ...
    def initialize_physical_parameters(self, d=4., T_cold=300., T_hot=700.):
        ...
        assert type(T_hot) == float, "T_hot must be a float"
        self.D = d
        self.T_cold = T_cold
        self.T_hot = T_hot
