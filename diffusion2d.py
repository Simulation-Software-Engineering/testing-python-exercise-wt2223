...
class SolveDiffusion2D:
    ...
    def initialize_domain(self, w=10., h=10., dx=0.1, dy=0.1):
        assert all(isinstance(x, float) for x in [w, h, dx, dy]), 'Please provide float numbers'
        self.w = w
        self.h = h
        self.dx = dx
        self.dy = dy
    ...
    def initialize_physical_parameters(self, d=4., T_cold=300., T_hot=700.):
        assert all(isinstance(x, float) for x in [d, T_cold, T_hot]), 'Please provide float numbers'
        self.D = d
        self.T_cold = T_cold
        self.T_hot = T_hot
